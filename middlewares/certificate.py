...
class CertificateValidator:
  ...
  def execute(self, headers: dict):
    ...
    if not client_cert: raise Exception('Send a certificate', 401)
    if not server_cert: raise Exception('No certificate provided for server', 401)
    ...
    if now < client_cert_decoded.not_valid_before or now > client_cert_decoded.not_valid_after:
      raise Exception('Send a certificate with valid date', 401)

  
  def __decode_cert(self, cert: str):
    key, cert, add  = pkcs12.load_key_and_certificates(base64.b64decode(cert), b"")
    return cert
